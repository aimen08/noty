...
# Finder scripting is the flakiest thing in this file and the layout is only
# cosmetic — a failure here must never cost a release its DMG.
osascript - "$MOUNT" <<'APPLESCRIPT' || echo "⚠ window layout skipped (Finder scripting unavailable)" >&2
on run argv
